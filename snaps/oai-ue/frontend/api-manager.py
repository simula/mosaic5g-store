...
# WITHOUT WARUETIES OR CONDITIONS OF ANY KIND, either express or implied.
...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of oai-ue", 
		  description = "Manage the manager of the OpenAPI of oai-ue",
          ordered = True,
...
api_change_host_port.add_argument('ue-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('ue-host', type=str, default="0.0.0.0", required=True)
...
class MainClassUeApiRestart(Resource):
...
class MainClassUeApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI oai-ue
        """        
...
class MainClassUeApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI oai-ue
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-ue.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassUeApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI oai-ue
        """        

...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of llmec", 
		  description = "Manage the manager of the OpenAPI of llmec",
          ordered = True,
...
api_change_host_port.add_argument('llmec-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('llmec-host', type=str, default="0.0.0.0", required=True)
...
class MainClassLlmecApiRestart(Resource):
...
class MainClassLlmecApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI llmec
        """        
...
class MainClassLlmecApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI llmec
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["llmec.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassLlmecApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI llmec
        """        

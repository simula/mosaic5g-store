...
# DIR for testing
# UPLOAD_FOLDER="/home/cigarier/Downloads/tmp/llmec/"
# PARAMETER_FOLDER="/home/cigarier/Downloads/tmp/llmec/parameters/"

flask_app = Flask(__name__.split('.')[0])                                                        
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "OpenAPI of llmec", 
		  description = "Manage the core network entity llmec throught OpenAPI ",
          ordered = True,
          terms_url= "https://www.openairinterface.org/?page_id=698",
          contact = "arouk@eurecom.fr, navid.nikaein@eurecom.fr")

llmec_space = flask_api.namespace('llmec', description='Management of llmec')
api_space = flask_api.namespace('llmec/api', description='Management of OpenAPI')
api_manager_space = flask_api.namespace('llmec/api-manager', description='Management of the manager of OpenAPI')


upload_conf_file = llmec_space.parser()
upload_conf_file.add_argument('file', location='files', type=FileStorage, required=False)


upload_set_conf_file = llmec_space.parser()
...
conf_show_file = llmec_space.parser()
...
api_change_host_port.add_argument('llmec-host', type=str, default="{}".format(api_host_default), required=True)
api_change_host_port.add_argument('llmec-port', type=str , default="{}".format(api_port_default), required=True)


api_manager_change_host_port = api_manager_space.parser()
api_manager_change_host_port.add_argument('llmec-host', type=str, default="{}".format(api_manager_host_default), required=True)
api_manager_change_host_port.add_argument('llmec-port', type=str , default="{}".format(api_manager_port_default), required=True)
...
## log
logger = logging.getLogger('llmec.openapi')
...
"""
Example Usage:
GET: curl http://0.0.0.0:5551/llmec/conf
POST: curl http://0.0.0.0:5551/llmec/conf  -X POST -F file=@llmec.conf
"""
@llmec_space.route("/conf")
class MainClassLlmecConf(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Get configuration file of llmec
        """
        # proc = subprocess.Popen(["llmec.conf-get"], stdout=subprocess.PIPE, shell=True)
        proc = subprocess.Popen(["$SNAP/conf echo"], stdout=subprocess.PIPE, shell=True)
    ...
    @llmec_space.expect(upload_set_conf_file)
    def post(self):
        """
        Upload/Set config file to llmec
        """
        ...
        if (type(config_file) == type(None)) and (type(uploaded_file) == type(None)):
            ...
            if (type(config_file) != type(None)):
                    ...
                    if allowed_file(config_file):
                        ...
                        if os.path.exists(file_absolute):
                            if os.path.isfile(file_absolute):
                                logger.debug("command to set config file= {} {}".format("$SNAP/conf set-llmec", file_absolute))
                                proc = subprocess.Popen(["$SNAP/conf set {}".format(file_absolute)], stdout=subprocess.PIPE, shell=True)
                                (out, err) = proc.communicate()
                ...
                if uploaded_file and allowed_file(uploaded_file.filename):
                    ...
                    if set_conf_file:
                        logger.debug("command to set config file= {} {}".format("$SNAP/conf set-llmec", file_absolute))
                        proc = subprocess.Popen(["$SNAP/conf set-llmec {}".format(file_absolute)], stdout=subprocess.PIPE, shell=True)
                        (out, err) = proc.communicate()
...
@llmec_space.route("/conf/list")
class MainClassLlmecConfList(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Get list of configuration files of llmec
        """        
        # proc = subprocess.Popen(["llmec.conf-list"], stdout=subprocess.PIPE, shell=True)
        proc = subprocess.Popen(["$SNAP/conf ls"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@llmec_space.route("/conf/show")
class MainClassLlmecConfShow(Resource):
    @llmec_space.expect(conf_show_file)
    @llmec_space.doc(params={"show-config-file": "Boolean value to indicate to show config file (true) of 'file-name' file (false). Default: True",
    ...
    def get(self):
        ...
        if args['show-config-file'] == False:
            ...
            if type(chosen_file) == type(None):
                ...
                proc = subprocess.Popen(["$SNAP/conf cat {}".format(chosen_file)], stdout=subprocess.PIPE, shell=True)
        else:
            if type(chosen_file) == type(None):
                # proc = subprocess.Popen(["llmec.conf-show"], stdout=subprocess.PIPE, shell=True)
                proc = subprocess.Popen(["$SNAP/conf cat-llmec"], stdout=subprocess.PIPE, shell=True)
            else:
                status_code = 400
...
@llmec_space.route("/init")
class MainClassLlmecInit(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Initialize the llmec to the default values
        """        
        proc = subprocess.Popen(["$SNAP/init ll-mec"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@llmec_space.route("/status")
class MainClassLlmecStatus(Resource):
    @llmec_space.produces(["application/json"])
    def get(self):
        """
        Get the status of llmec and api
        """        
        proc = subprocess.Popen(["$SNAP/run status ll-mecd"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["ll-mec.status"], stdout=subprocess.PIPE, shell=True)
        ...
        return response
@llmec_space.route("/status-all")
class MainClassLlmecStatusAll(Resource):
    @llmec_space.produces(["application/json"])
    def get(self):
        """
        ...
        # proc = subprocess.Popen(["llmec.status"], stdout=subprocess.PIPE, shell=True)
...
@llmec_space.route("/start")
class MainClassLlmecStart(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Start the service ll-mec in deamon mode
        """        
        proc = subprocess.Popen(["$SNAP/run start ll-mecd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@llmec_space.route("/stop")
class MainClassLlmecStop(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Stop the service ll-mec in deamon mode
        """        
        proc = subprocess.Popen(["$SNAP/run stop ll-mecd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 

        response = make_response(out)
        response.headers.set("Content-Type", "text")
        return response
@llmec_space.route("/restart")
class MainClassLlmecReStart(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Restart the service ll-mec in deamon mode
        """        
        proc = subprocess.Popen(["$SNAP/run restart ll-mecd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@llmec_space.route("/journal")
class MainClassLlmecJournal(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
        Get the journal of ll-mec
        """        
        proc = subprocess.Popen(["$SNAP/run journal ll-mecd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
        ...
        if os.path.exists(config_file) and os.path.isfile(config_file):
            try:
                with open(config_file) as f:
                    ...
                    api_config_param = {
                        'llmec-host': host_llmec,
                        'llmec-port': port_llmec
                    }
                    """
            except:
                api_config_param = {
                    'llmec-host': api_host_default,
                    'llmec-port': api_port_default
                }  
        else:
            api_config_param = {
                    'llmec-host': api_host_default,
...
class MainClassLlmecApiJournal(Resource):
    @llmec_space.produces(["text"])
    def get(self):
        """
    ...
    def get(self):
        ...
        if os.path.exists(config_file) and os.path.isfile(config_file):
            try:
                ...
                api_config_param = {
                    'llmec-host': api_manager_host_default,
                    'llmec-port': api_manager_port_default
                }  
        else:
            api_config_param = {
                    'llmec-host': api_manager_host_default,
...
class MainClassLlmecApiManagerStart(Resource):
    ...
    def put(self):
        ...
        new_url = '{}//{}:{}'.format(new_url[0], host_llmec, port_llmec)
...
class MainClassLlmecApiManagerStop(Resource):
    @api_manager_space.produces(["application/json"])
    def get(self):
        """
        Stop the manager of OpenAPI llmec
        """        
...
class MainClassLlmecApiManagerRestart(Resource):
    @api_manager_space.produces(["text"])
    @api_manager_space.expect(api_manager_change_host_port)
    @api_manager_space.doc(params={
                "llmec-host": "Valid IP address of llmec",
    ...
    def put(self):
        """
        Restart the manager of OpenAPI llmec
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_manager_change_host_port.parse_args()
        host_llmec = args['llmec-host']
...
class MainClassLlmecApiManagerStatus(Resource):
    @api_manager_space.produces(["application/json"])
    def get(self):
        """
        Get the status of the manager of OpenAPI llmec
        """        
        proc = subprocess.Popen(["$SNAP/run status apidman"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["llmec.apiman-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassLlmecApiManagerJournal(Resource):
    @api_manager_space.produces(["text"])
    def get(self):
        """
        Get the journal of the manager of OpenAPI llmec
        """        
...
if __name__ == "__main__":                                                     
    ...
    parser = argparse.ArgumentParser(description='provide host and port for flask api of llmec')
        
    parser.add_argument('--llmec-host', metavar='[option]', action='store', type=str,
                        required=False, default='{}'.format(api_host_default), 
                        help='Set OpenAPI-LLMEC IP address to bind to, {} (default)'.format(api_host_default))
    
    parser.add_argument('--llmec-port', metavar='[option]', action='store', type=str,
                        required=False, default='{}'.format(api_port_default), 
                        help='Set llmec port number: {} (default)'.format(api_port_default))
    ...
    if os.path.exists(config_file) and os.path.isfile(config_file):
        with open(config_file) as f:
            ...
            """
            flask_app.run(host=config_param["llmec-host"], port=config_param["llmec-port"], debug=True)
    else:

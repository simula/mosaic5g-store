...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of oai-mme", 
		  description = "Manage the manager of the OpenAPI of oai-mme",
          ordered = True,
...
api_change_host_port.add_argument('mme-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('mme-host', type=str, default="0.0.0.0", required=True)
...
class MainClassMmeApiRestart(Resource):
...
class MainClassMmeApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI oai-mme
        """        
...
class MainClassMmeApiManagerRestart(Resource):
    ...
    def put(self):
        """
        Restart the OpenAPI of oai-mme
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_mme = args['mme-host']
...
class MainClassMmeApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI oai-mme
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-mme.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassMmeApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI oai-mme
        """        

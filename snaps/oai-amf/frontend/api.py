...
# DIR for testing
# UPLOAD_FOLDER="/home/cigarier/Downloads/tmp/oai-amf/"
# PARAMETER_FOLDER="/home/cigarier/Downloads/tmp/oai-amf/parameters/"

flask_app = Flask(__name__.split('.')[0])                                                        
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "OpenAPI of oai-amf", 
		  description = "Manage the core network entity oai-amf throught OpenAPI ",
          ordered = True,
          terms_url= "https://www.openairinterface.org/?page_id=698",
          contact = "arouk@eurecom.fr, navid.nikaein@eurecom.fr")

amf_space = flask_api.namespace('amf', description='Management of oai-amf')
api_space = flask_api.namespace('amf/api', description='Management of OpenAPI')
api_manager_space = flask_api.namespace('amf/api-manager', description='Management of the manager of OpenAPI')


upload_conf_file = amf_space.parser()
upload_conf_file.add_argument('file', location='files', type=FileStorage, required=False)


upload_set_conf_file = amf_space.parser()
...
conf_show_file = amf_space.parser()
...
api_change_host_port.add_argument('amf-host', type=str, default="{}".format(api_host_default), required=True)
api_change_host_port.add_argument('amf-port', type=str , default="{}".format(api_port_default), required=True)


api_manager_change_host_port = api_manager_space.parser()
api_manager_change_host_port.add_argument('amf-host', type=str, default="{}".format(api_manager_host_default), required=True)
api_manager_change_host_port.add_argument('amf-port', type=str , default="{}".format(api_manager_port_default), required=True)
...
## log
logger = logging.getLogger('amf.openapi')
...
"""
Example Usage:
GET: curl http://0.0.0.0:5551/amf/conf
POST: curl http://0.0.0.0:5551/amf/conf  -X POST -F file=@amf.conf
"""
@amf_space.route("/conf")
class MainClassAmfConf(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Get configuration file of oai-amf
        """
        # proc = subprocess.Popen(["oai-amf.conf-get"], stdout=subprocess.PIPE, shell=True)
        proc = subprocess.Popen(["$SNAP/conf echo-amf"], stdout=subprocess.PIPE, shell=True)
    ...
    @amf_space.expect(upload_set_conf_file)
    def post(self):
        """
        Upload/Set config file to oai-amf
        """
                    ...
                    if allowed_file(config_file):
                        ...
                        if os.path.exists(file_absolute):
                            if os.path.isfile(file_absolute):
                                logger.debug("command to set config file= {} {}".format("$SNAP/conf set-amf", file_absolute))
                                proc = subprocess.Popen(["$SNAP/conf set-amf {}".format(file_absolute)], stdout=subprocess.PIPE, shell=True)
                                (out, err) = proc.communicate()
...
@amf_space.route("/conf/list")
class MainClassAmfConfList(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Get list of configuration files of oai-amf
        """        
        # proc = subprocess.Popen(["oai-amf.conf-list"], stdout=subprocess.PIPE, shell=True)
        proc = subprocess.Popen(["$SNAP/conf ls-amf"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@amf_space.route("/conf/show")
class MainClassAmfConfShow(Resource):
    @amf_space.expect(conf_show_file)
    @amf_space.doc(params={"show-config-file": "Boolean value to indicate to show config file (true) of 'file-name' file (false). Default: True",
    ...
    def get(self):
        ...
        if args['show-config-file'] == False:
            ...
            if type(chosen_file) == type(None):
                ...
                proc = subprocess.Popen(["$SNAP/conf cat-amf {}".format(chosen_file)], stdout=subprocess.PIPE, shell=True)
        else:
            if type(chosen_file) == type(None):
                # proc = subprocess.Popen(["oai-amf.conf-show"], stdout=subprocess.PIPE, shell=True)
                proc = subprocess.Popen(["$SNAP/conf cat-amf"], stdout=subprocess.PIPE, shell=True)
            else:
                status_code = 400
...
@amf_space.route("/init")
class MainClassAmfInit(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Initialize the oai-amf to the default values
        """        
        proc = subprocess.Popen(["$SNAP/init amf"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@amf_space.route("/status")
class MainClassAmfStatus(Resource):
    @amf_space.produces(["application/json"])
    def get(self):
        """
        Get the status of amf and api
        """        
        proc = subprocess.Popen(["$SNAP/run status amfd"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-amf.status"], stdout=subprocess.PIPE, shell=True)
        ...
        return response
@amf_space.route("/status-all")
class MainClassAmfStatusAll(Resource):
    @amf_space.produces(["application/json"])
    def get(self):
        """
...
@amf_space.route("/start")
class MainClassAmfStart(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Start the service oai-amf in deamon mode
        """        
        proc = subprocess.Popen(["$SNAP/run start amfd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@amf_space.route("/stop")
class MainClassAmfStop(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Stop the service oai-amf in deamon mode
        """        
        proc = subprocess.Popen(["$SNAP/run stop amfd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 

        response = make_response(out)
        response.headers.set("Content-Type", "text")
        return response
@amf_space.route("/restart")
class MainClassAmfReStart(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Restart the service oai-amf in deamon mode
        """        
        proc = subprocess.Popen(["$SNAP/run restart amfd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
...
@amf_space.route("/journal")
class MainClassAmfJournal(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
        Get the journal of oai-amf
        """        
        proc = subprocess.Popen(["$SNAP/run journal amfd"], stdout=subprocess.PIPE, shell=True)
        (out, err) = proc.communicate() 
    ...
    def get(self):
        ...
        if os.path.exists(config_file) and os.path.isfile(config_file):
            try:
                ...
                api_config_param = {
                    'amf-host': api_host_default,
                    'amf-port': api_port_default
                }  
        else:
            api_config_param = {
                    'amf-host': api_host_default,
...
class MainClassAmfApiJournal(Resource):
    @amf_space.produces(["text"])
    def get(self):
        """
    ...
    def get(self):
        ...
        if os.path.exists(config_file) and os.path.isfile(config_file):
            try:
                ...
                api_config_param = {
                    'amf-host': api_manager_host_default,
                    'amf-port': api_manager_port_default
                }  
        else:
            api_config_param = {
                    'amf-host': api_manager_host_default,
...
class MainClassAmfApiManagerStart(Resource):
    ...
    def put(self):
        ...
        new_url = '{}//{}:{}'.format(new_url[0], host_amf, port_amf)
...
class MainClassAmfApiManagerStop(Resource):
    @api_manager_space.produces(["application/json"])
    def get(self):
        """
        Stop the manager of OpenAPI oai-amf
        """        
...
class MainClassAmfApiManagerRestart(Resource):
    @api_manager_space.produces(["text"])
    @api_manager_space.expect(api_manager_change_host_port)
    @api_manager_space.doc(params={
                "amf-host": "Valid IP address of oai-amf",
    ...
    def put(self):
        """
        Restart the manager of OpenAPI oai-amf
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_manager_change_host_port.parse_args()
        host_amf = args['amf-host']
...
class MainClassAmfApiManagerStatus(Resource):
    @api_manager_space.produces(["application/json"])
    def get(self):
        """
        Get the status of the manager of OpenAPI oai-amf
        """        
        proc = subprocess.Popen(["$SNAP/run status apidman"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-amf.apiman-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassAmfApiManagerJournal(Resource):
    @api_manager_space.produces(["text"])
    def get(self):
        """
        Get the journal of the manager of OpenAPI oai-amf
        """        
...
if __name__ == "__main__":                                                     
    ...
    parser = argparse.ArgumentParser(description='provide host and port for flask api of oai-amf')
        
    parser.add_argument('--amf-host', metavar='[option]', action='store', type=str,
                        required=False, default='{}'.format(api_host_default), 
                        help='Set OpenAPI-AMF IP address to bind to, {} (default)'.format(api_host_default))
    
    parser.add_argument('--amf-port', metavar='[option]', action='store', type=str,
                        required=False, default='{}'.format(api_port_default), 
                        help='Set oai-amf port number: {} (default)'.format(api_port_default))
    ...
    if os.path.exists(config_file) and os.path.isfile(config_file):
        with open(config_file) as f:
            ...
            """
            flask_app.run(host=config_param["amf-host"], port=config_param["amf-port"], debug=True)
    else:

...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of oai-amf", 
		  description = "Manage the manager of the OpenAPI of oai-amf",
          ordered = True,
...
api_change_host_port.add_argument('amf-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('amf-host', type=str, default="0.0.0.0", required=True)
...
class MainClassAmfApiRestart(Resource):
...
class MainClassAmfApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI oai-amf
        """        
...
class MainClassAmfApiManagerRestart(Resource):
    ...
    def put(self):
        """
        Restart the OpenAPI of oai-amf
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_amf = args['amf-host']
...
class MainClassAmfApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI oai-amf
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-amf.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassAmfApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI oai-amf
        """        

...
api_port_default = 5530

api_manager_host_default = '0.0.0.0'
api_manager_port_default = 6630

flask_app = Flask(__name__.split('.')[0])                                                        
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of flexran", 
		  description = "Manage the manager of the OpenAPI of flexran",
          ordered = True,
...
api_change_host_port.add_argument('api-flexran-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('api-manager-flexran-host', type=str, default="0.0.0.0", required=True)
        ...
        if os.path.exists(config_file) and os.path.isfile(config_file):
            try:
                with open(config_file) as f:
                    ...
                    api_config_param = {
                        'api-manager-flexran-host': host_flexran,
                        'api-manager-flexran-port': port_flexran
                    }
                    """
            except:
                api_config_param = {
                    'api-manager-flexran-host': '0.0.0.0',
...
class MainClassFlexranApiManagerJournal(Resource):
...
class MainClassFlexranApiConf(Resource):
        ...
        if os.path.exists(config_file) and os.path.isfile(config_file):
            try:
                with open(config_file) as f:
                    ...
                    # config file is of the form
                    api_config_param = {
                        'api-flexran-host': host_flexran,
                        'api-flexran-port': port_flexran
                    }
                    """
            except:
                api_config_param = {
                    'api-flexran-host': '0.0.0.0',
...
class MainClassFlexranApiRestart(Resource):
    @api_space.produces(["text"])
    @api_space.expect(api_change_host_port)
    @api_space.doc(params={
                "api-flexran-host": "Valid IP address of flexran",
                "api-flexran-port": "Valid port of flexran"
    })
    def put(self):
        """
        Start OpenAPI flexran
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_flexran = args['api-flexran-host']
        port_flexran =  args['api-flexran-port']
        ...
        new_url = '{}//{}:{}'.format(new_url[0], host_flexran, port_flexran)
...
class MainClassFlexranApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI flexran
        """        
...
class MainClassFlexranApiManagerRestart(Resource):
    @api_space.produces(["text"])
    @api_space.expect(api_change_host_port)
    @api_space.doc(params={
                "api-flexran-host": "Valid IP address of flexran",
    ...
    def put(self):
        """
        Restart the OpenAPI of flexran
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_flexran = args['api-flexran-host']
...
class MainClassFlexranApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI flexran
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["flexran.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassFlexranApiJournal(Resource):
    @api_space.produces(["text"])
    def get(self):
        """
        Get the journal of the OpenAPI flexran
        """        
...
if __name__ == "__main__":                                                     
    ...
    parser.add_argument('--api-manager-flexran-host', metavar='[option]', action='store', type=str,
                        required=False, default='0.0.0.0', 
                        help='Set OpenAPI-MANAGER IP address to bind to, 0.0.0.0 (default)')
    
    parser.add_argument('--api-manager-flexran-port', metavar='[option]', action='store', type=str,
    ...
    if os.path.exists(config_file) and os.path.isfile(config_file):
        with open(config_file) as f:
            ...
            flask_app.run(host=config_param["api-manager-flexran-host"], port=config_param["api-manager-flexran-port"], debug=True)
    else:

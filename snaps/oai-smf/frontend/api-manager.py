...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of oai-smf", 
		  description = "Manage the manager of the OpenAPI of oai-smf",
          ordered = True,
...
api_change_host_port.add_argument('smf-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('smf-host', type=str, default="0.0.0.0", required=True)
...
class MainClassSmfApiRestart(Resource):
...
class MainClassSmfApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI oai-smf
        """        
...
class MainClassSmfApiManagerRestart(Resource):
    ...
    def put(self):
        """
        Restart the OpenAPI of oai-smf
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_smf = args['smf-host']
...
class MainClassSmfApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI oai-smf
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-smf.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassSmfApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI oai-smf
        """        

...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of oai-spgwc", 
		  description = "Manage the manager of the OpenAPI of oai-spgwc",
          ordered = True,
          terms_url= "https://www.openairinterface.org/?page_id=698",
          contact = "arouk@eurecom.fr, navid.nikaein@eurecom.fr")

spgwc_space = flask_api.namespace('spgwc', description='Management of oai-spgwc')
api_space = flask_api.namespace('spgwc/api', description='Management of OpenAPI')
...
api_change_host_port.add_argument('spgwc-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('spgwc-host', type=str, default="0.0.0.0", required=True)
...
class MainClassSpgwcApiRestart(Resource):
...
class MainClassSpgwcApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI oai-spgwc
        """        
...
class MainClassSpgwcApiManagerRestart(Resource):
    ...
    def put(self):
        """
        Restart the OpenAPI of oai-spgwc
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_spgwc = args['spgwc-host']
...
class MainClassSpgwcApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI oai-spgwc
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-spgwc.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassSpgwcApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI oai-spgwc
        """        

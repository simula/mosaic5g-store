...
flask_api = Api(flask_app, 
		  version = "1.0", 
		  title = "Manager the OpenAPI of oai-spgwu", 
		  description = "Manage the manager of the OpenAPI of oai-spgwu",
          ordered = True,
          terms_url= "https://www.openairinterface.org/?page_id=698",
          contact = "arouk@eurecom.fr, navid.nikaein@eurecom.fr")

spgwu_space = flask_api.namespace('spgwu', description='Management of oai-spgwu')
api_space = flask_api.namespace('spgwu/api', description='Management of OpenAPI')
...
api_change_host_port.add_argument('spgwu-host', type=str, default="0.0.0.0", required=True)
...
api_manager_change_host_port.add_argument('spgwu-host', type=str, default="0.0.0.0", required=True)
...
class MainClassSpgwuApiRestart(Resource):
...
class MainClassSpgwuApiStop(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Stop the OpenAPI oai-spgwu
        """        
...
class MainClassSpgwuApiManagerRestart(Resource):
    ...
    def put(self):
        """
        Restart the OpenAPI of oai-spgwu
        WARNING: you may loose the connection if you enter non valid parameters
        """     
        args = api_change_host_port.parse_args()
        host_spgwu = args['spgwu-host']
...
class MainClassSpgwuApiStatus(Resource):
    @api_space.produces(["application/json"])
    def get(self):
        """
        Get the status of OpenAPI oai-spgwu
        """        
        proc = subprocess.Popen(["$SNAP/run status apid"], stdout=subprocess.PIPE, shell=True)
        # proc = subprocess.Popen(["oai-spgwu.api-journal"], stdout=subprocess.PIPE, shell=True)
...
class MainClassSpgwuApiJournal(Resource):
    ...
    def get(self):
        """
        Get the journal of the OpenAPI oai-spgwu
        """        

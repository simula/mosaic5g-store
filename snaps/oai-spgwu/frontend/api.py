...
"""
Example Usage:
GET: curl http://0.0.0.0:5554/spgwu/conf
POST: curl http://0.0.0.0:5554/spgwu/conf  -X POST -F file=@spgwu.conf
"""
...
if __name__ == "__main__":                                                     
    ...
    parser = argparse.ArgumentParser(description='Pass host and port for flask api of spgwu')
        
    parser.add_argument('--spgwu-host', metavar='[option]', action='store', type=str,
                        required=False, default='0.0.0.0', 
                        help='Set OpenAPI-SPGWU IP address to bind to, 0.0.0.0 (default)')
    
    parser.add_argument('--spgwu-port', metavar='[option]', action='store', type=str,
                        required=False, default='5554', 
                        help='Set spgwu port number: 5554 (default)')
    args = parser.parse_args()
    flask_app.run(host=args.spgwu_host, port=args.spgwu_port, debug=True)
    #############################
    # # new way to change the config of api by taking them from config file. like the config file of spgwu
    ...
    # except:
    #     flask_app.run(host='0.0.0.0', port='5554', debug=True)